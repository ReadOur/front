...
- [x] **책 리뷰 게시판 기능 구현** ✅ **완료**
  - ✅ Post 타입에 `bookId` 필드 추가
  - ✅ BRD_05.tsx: 책 정보 카드 표시 (표지, 제목, 저자)
  - ✅ BRD_06.tsx: 책 ID 입력 필드 (REVIEW 카테고리)
  - ✅ 책 표지 클릭 시 책 상세 페이지 이동 연결
- [x] **모임 게시판 기능 구현** ✅ **완료 (백엔드 API 대기 중)**
  - ✅ Post 타입에 `chatRoomId`, `currentMemberCount`, `recruitmentLimit`, `isApplied` 필드 추가
  - ✅ BRD_05.tsx: 채팅방 입장 버튼, 참여 인원 표시
  - ✅ BRD_06.tsx: 채팅방 ID, 모집 인원 입력 필드 (DISCUSSION 카테고리)
  - ⚠️ 백엔드 API에서 추가 필드 지원 필요 (chatRoomId, currentMemberCount, recruitmentLimit, isApplied)
...
- [x] **좋아요 누른 글 조회 API 연동** (Line 148-149, 페이징) ✅ **완료**
  - ✅ `getMyLikedPosts()` 서비스 함수 구현
  - ✅ `useMyLikedPosts()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
- [x] **내가 작성한 게시글 목록 API 연동** (Line 150-151, 페이징) ✅ **완료**
  - ✅ `getMyPosts()` 서비스 함수 구현
  - ✅ `useMyPosts()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
- [x] **내가 작성한 댓글 목록 API 연동** (Line 152-153, 페이징) ✅ **완료**
  - ✅ `getMyComments()` 서비스 함수 구현
  - ✅ `useMyComments()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
- [x] **특정 사용자 마이페이지 조회 API 연동** (Line 157-158) ✅ **완료**
  - ✅ `getUserProfile()` 서비스 함수 구현
  - ✅ `useUserProfile()` 훅 구현
  - ✅ `PRF_10.tsx` 페이지 연동 완료 (userId 기반 분기)
- [x] **특정 사용자 작성 게시글 조회 API 연동** (Line 159-160) ✅ **완료**
  - ✅ `getUserPosts()` 서비스 함수 구현
  - ✅ `useUserPosts()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
- [x] **특정 사용자 좋아요 누른 글 조회 API 연동** (Line 161-162) ✅ **완료**
  - ✅ `getUserLikedPosts()` 서비스 함수 구현
  - ✅ `useUserLikedPosts()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
- [x] **특정 사용자 작성 댓글 조회 API 연동** (Line 163-164) ✅ **완료**
  - ✅ `getUserComments()` 서비스 함수 구현
  - ✅ `useUserComments()` 훅 구현
  - ✅ `PRF_10.tsx` 전체보기 페이지 연동 완료
...
### 📁 파일 관리 ✅ **전부 완료**
- [x] **파일 업로드 프론트엔드 연동** (Line 276-277) ✅ **완료**
  - ✅ `uploadFile()`, `uploadFiles()` 서비스 함수 구현
  - ✅ FileUpload 컴포넌트 구현 (드래그 앤 드롭, 진행률 표시)
  - ✅ BRD_06.tsx: 게시글 작성 시 파일 첨부 기능 연동 완료
- [x] **파일 메타데이터 조회 프론트엔드 연동** (Line 278-279) ✅ **완료**
  - ✅ `getDownloadUrl()`, `formatFileSize()`, `isImageFile()` 헬퍼 함수 구현
  - ✅ BRD_05.tsx: 첨부파일 목록 및 메타데이터 표시 완료
- [x] **파일 다운로드 프론트엔드 연동** (Line 280-281) ✅ **완료**
  - ✅ `getDownloadUrl()` 함수로 다운로드 URL 생성
  - ✅ BRD_05.tsx: 다운로드 버튼 연동 완료

### 💬 채팅
- [x] **채팅방 목록 조회 프론트엔드 연동** (Line 330-331) ✅ **완료**
  - ✅ `getRoomsOverview()` 서비스 함수 구현
  - ✅ `useRoomsOverview()` 훅 구현
  - ✅ CHT_17.tsx: 채팅방 목록 페이지 연동 완료
- [x] **내 채팅방 목록 조회 프론트엔드 연동** (Line 332-333) ✅ **완료**
  - ✅ `getMyRooms()` 서비스 함수 구현
  - ✅ `useMyRooms()` 훅 구현
  - ✅ ChatDock.tsx: Floating 채팅 도크 연동 완료
- [x] **채팅 메시지 타임라인 조회 프론트엔드 연동** (Line 352-353) ✅ **완료**
  - ✅ `getRoomMessages()` 서비스 함수 구현
  - ✅ ChatDock.tsx: 메시지 목록 표시 완료
- [x] **채팅 메시지 전송 프론트엔드 연동** (Line 354-355) ✅ **완료**
  - ✅ `sendRoomMessage()` 서비스 함수 구현
  - ✅ `useSendRoomMessage()` 훅 구현
  - ✅ ChatDock.tsx: 메시지 전송 기능 연동 완료
- [x] **게시판에서 채팅방 자동 입장 기능** ✅ **완료**
  - ✅ BRD_05.tsx: 채팅방 입장 버튼 클릭 시 `/chat?roomId=X` 이동
  - ✅ CHT_17.tsx: roomId 쿼리 파라미터 감지 및 자동 오픈
  - ✅ ChatDock: 우측 하단 floating 창에서 채팅방 표시
...
### 📚 내서재
- [ ] **도서 검색 (외부 API) 프론트엔드 연동** (Line 177-178) ⚠️ **백엔드 미완성**
  - ❌ LibrarySearch.tsx에서 mock 데이터 사용 중
  - ❌ 백엔드 API 구현 대기 중
- [ ] **특정 사용자 서재 조회** (Line 227-228) ⚠️ **백엔드 미완성**
  - ❌ 백엔드 API 구현이 불완전하다고 확인됨
