...
export default function HeaderApp({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 w-full border-b border-[color:var(--color-border-subtle)] text-[color:var(--color-fg-primary)] z-50" style={{ backgroundColor: '#F5EFE9', opacity: 1 }}>
      {/* 가운데 정렬 컨테이너 */}
      <div className="mx-auto px-4 h-32" style={{ maxWidth: "var(--layout-max)" }}>
        ...
        <div className="grid grid-cols-[auto,1fr,auto] items-center gap-4 h-full">
          ...
          <button onClick={onLogoClick} className="shrink-0 flex items-center">
            <div
              className="flex items-center scale-[1.32]"
          ...
          <nav className="flex justify-center">
            <ul className="flex items-stretch h-full font-medium text-[color:var(--color-fg-muted)] text-xl">
              {navItems.map((item, idx) => (
                <li key={item.key} className="relative flex items-center px-16">
                  <button
          ...
          {/* RIGHT: 유틸리티 버튼 */}
          <div className="shrink-0 flex items-center gap-[15px] justify-end pb-4" style={{ position: 'relative', top: '0', marginTop: '-0.5rem' }}>
            {/* 채팅 버튼 */}
            <button
              onClick={onClickChat}
              className="relative w-14 h-14 grid place-items-center rounded-[var(--radius-md)] border border-[color:var(--color-border-subtle)] bg-[color:var(--color-bg-elev-1)] hover:bg-[color:var(--color-bg-hover)] transition-colors cursor-pointer"
              aria-label="채팅"
            >
              <MessageCircle className="w-6 h-6 text-[color:var(--color-fg-primary)]" />
              {chatUnreadCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-6 h-6 px-1 grid place-items-center rounded-full text-xs font-bold bg-[color:var(--color-accent)] text-[color:var(--color-on-accent)]">
